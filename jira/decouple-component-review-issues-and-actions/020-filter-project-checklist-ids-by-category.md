...
When a user types or selects a Category value on a review item, the available Checklist ID choices for that item should be filtered to checklist issues in that category. Category values come from the Component Review Issues administration data. New categories are maintained in the Issues administration screen, not in this project view.
...
- Checklist ID choices are filtered by the Category value on that row.
